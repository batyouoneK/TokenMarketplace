...
class Miner {
    ...
    public static function add_miner_to_plans($name, $value, $profit, $end, $username) {
        ...
        $datass = '1234567890ABCDEFGHIJKLMNOPQRSTUVWXYZabcefghijklmnopqrstuvwxyz';
        $passfunc = substr(str_shuffle($datass), 0, 10);
        $passfuncson = "TX_" . $passfunc;
 
        $sql = "INSERT INTO plans (name, value, profit, end, author, status, buy_date, txid) ";
        $sql .= "VALUES ('{$name}','{$value}','{$profit}','{$end}','{$author}', '{$status}', '{$buy_date}','{$passfuncson}')";
